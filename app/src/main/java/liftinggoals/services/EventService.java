...
public class EventService extends IntentService {
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        if (intent != null)
        {
            ...
            workout.setWorkoutName(intent.getStringExtra("workoutName"));
            workout.setDescription(intent.getStringExtra("description"));
            workout.setNumberExercises(intent.getIntExtra("numExercises", -1));
            workout.setEstimatedDuration(intent.getDoubleExtra("duration", -1));
    ...
    private void updateWorkout()
    {
        ...
        final StringRequest stringRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                db.closeDB();
                Intent intent = new Intent("errorWorkoutAction");
                LocalBroadcastManager.getInstance(getApplicationContext()).sendBroadcast(intent);
                Toast.makeText(getApplicationContext(), "Error updating Workout", Toast.LENGTH_LONG).show();
            }
        }){
